...
[data-testid="stSidebar"] .stMarkdown { color: #4a4a4a !important; }
...
[data-testid="stTab"]:hover { color: #1e3a5f; }
...
::-webkit-scrollbar-thumb:hover { background: #a8a095; }
...
.stButton > button:hover {
    ...
    box-shadow: 0 4px 16px rgba(30,58,95,0.20) !important;
...
# ─── COLOUR PALETTE (Decent & Professional) ─────────────────────────────────
# These are used consistently across all charts and UI elements.
# Each colour is chosen for clarity, accessibility, and visual harmony.

NAVY      = "#1e3a5f"   # Primary — headings, main bars, trust
TEAL      = "#2a9d8f"   # Accent — secondary data, calm contrast
FOREST    = "#2d6a4f"   # Success — positive EBITDA, profit
AMBER     = "#b45309"   # Warning — medium variance, caution
BRICK     = "#9b2226"   # Danger — negative EBITDA, high variance
SLATE     = "#475569"   # Neutral — grid lines, borders
WARM_GREY = "#6b7280"   # Muted text, secondary info
SURFACE   = "#faf9f7"   # Background surface

# Chart colour cycle — ordered for maximum distinguishability
CHART_COLORS = [NAVY, TEAL, FOREST, AMBER, BRICK, "#5c7aea", "#e76f51", "#264653", "#e9c46a", "#2a9d8f"]

# Transparent backgrounds for Plotly
...
def base_layout(**kwargs):
    """Return a consistent Plotly layout with our decent theme."""
    return dict(
...
def styled_axis(title="", fmt="", showgrid=True):
    """Return a consistently styled axis dictionary."""
    return dict(
        title=title,
...
def load_data(filepath) -> pd.DataFrame:
    ...
    # Safe percentage calculations — protect against divide-by-zero
    ...
    # ── EBITDA sign indicator ──
    df["EBITDA_SIGN"] = df["KITCHEN EBITDA"].apply(
...
def render_sidebar(df: pd.DataFrame):
    """Render the sidebar with brand, dashboard selector, and all filters."""
    st.sidebar.markdown("""
    <div class='brand'>
    ...
    st.sidebar.markdown("---")
    st.sidebar.markdown(
        "<div style='text-align:center; color:#9ca3af; font-size:0.68rem;'>"
        "Data refreshes every 5 min<br>📊 Kitchen P&L Suite v3.1</div>",
        unsafe_allow_html=True
    )
...
def apply_var_filters(df: pd.DataFrame, f: dict) -> pd.DataFrame:
    """Apply all Variance Level P&L filters to the dataframe."""
    mask = pd.Series([True] * len(df), index=df.index)
...
def render_kitchen_pnl(df: pd.DataFrame, filters: dict):
    """Render the Kitchen Level P&L dashboard with overview, trends, store analysis, and data."""
...
def render_overview(fdf: pd.DataFrame):
    """Render the Overview tab with monthly charts and EBITDA distribution."""
    ...
    with col1:
        ...
        fig_pie.update_layout(
            **base_layout(title="Positive vs Negative EBITDA"),
            height=350,
            annotations=[dict(text="EBITDA", x=0.5, y=0.5, font_size=16, showarrow=False, font_color="#1e3a5f")]
        )
    ...
    with col2:
        ...
        fig_hist = go.Figure(go.Histogram(
            ...
            marker_color=NAVY, opacity=0.7,
            hovertemplate="EBITDA: ₹%{x:.1f}L<br>Stores: %{y}<extra></extra>"
        ))
        fig_hist.add_vline(x=0, line_dash="dash", line_color=BRICK, line_width=2)
...
def render_trends(fdf: pd.DataFrame):
    """Render the Trends tab with monthly trend lines and city-wise comparison."""
    ...
    fig = make_subplots(
        rows=2, cols=1, 
        subplot_titles=("Revenue Trend", "Margin % Trend"),
        vertical_spacing=0.15
    )

    fig.add_trace(go.Scatter(
        x=monthly["MONTH_STR"], y=monthly["NET REVENUE"]/1e5,
        mode="lines+markers", name="Revenue", line=dict(color=NAVY, width=3),
        marker=dict(size=10), fill="tozeroy", fillcolor=hex_to_rgba(NAVY, 0.08),
        hovertemplate="Month: %{x}<br>Revenue: ₹%{y:.1f}L<extra></extra>"
    ), row=1, col=1)

    fig.add_trace(go.Scatter(
        x=monthly["MONTH_STR"], y=monthly["GM%"],
        mode="lines+markers", name="GM%", line=dict(color=TEAL, width=3),
        marker=dict(size=8), hovertemplate="Month: %{x}<br>GM%: %{y:.1f}%<extra></extra>"
    ), row=2, col=1)

    fig.add_trace(go.Scatter(
        x=monthly["MONTH_STR"], y=monthly["EBITDA%"],
        mode="lines+markers", name="EBITDA%", line=dict(color=FOREST, width=3),
    ...
    fig_city.add_trace(go.Bar(
        ...
        orientation="h", name="Revenue", marker_color=NAVY, opacity=0.85,
        hovertemplate="%{y}<br>Revenue: ₹%{x:.1f}L<extra></extra>"
    ))
    fig_city.add_trace(go.Scatter(
        y=city_perf["CITY"], x=city_perf["EBITDA%"],
        mode="markers", name="EBITDA%", marker=dict(color=FOREST, size=12, symbol="diamond"),
    ...
    fig_city.update_layout(
        **base_layout(title="Revenue & EBITDA% by City", height=500),
        ...
        xaxis2=dict(overlaying="x", side="top", title="EBITDA %", tickfont=dict(color="#6b7280")),
        yaxis=dict(tickfont=dict(size=11, color="#2c2c2c"))
    )
    st.plotly_chart(fig_city, use_container_width=True)


def render_store_analysis(fdf: pd.DataFrame):
    """Render the Store Analysis tab with performance matrix and top/bottom lists."""
...
def render_data_table(fdf: pd.DataFrame):
    """Render the raw data table with formatted values and a download button."""
    st.markdown('<div class="section-header">Raw Data</div>', unsafe_allow_html=True)

    display_cols = [
        "MONTH_STR", "STORE", "CITY", "ZONE MAPPING", "STATUS",
...
def render_variance_pnl(df: pd.DataFrame, filters: dict):
    """Render the Variance Level P&L dashboard with distribution, analysis, and data."""
    ...
    with st.expander("📊 View Full Insights Summary", expanded=False):
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
            <div style="background: #f5f3ef; border: 1px solid #e5e0d8; border-radius: 12px; padding: 16px;">
                <h4 style="color: #1e3a5f; margin: 0 0 8px 0; font-size: 0.95rem;">📅 Monthly Performance</h4>
                <p style="color: #4a4a4a; margin: 4px 0; font-size: 0.85rem;">
                    <b>Best Month</b>: <code style="background: #ffffff; padding: 2px 6px; border-radius: 4px; color: #1e3a5f;">{best_month}</code> 
                    — ₹{best_ebitda/1e7:.2f} Cr total EBITDA
                </p>
                <p style="color: #4a4a4a; margin: 4px 0; font-size: 0.85rem;">
                    <b>Worst Month</b>: <code style="background: #ffffff; padding: 2px 6px; border-radius: 4px; color: #1e3a5f;">{worst_month}</code> 
                    — ₹{worst_ebitda/1e7:.2f} Cr total EBITDA
                </p>
            </div>
            <div style="background: #f5f3ef; border: 1px solid #e5e0d8; border-radius: 12px; padding: 16px;">
                <h4 style="color: #1e3a5f; margin: 0 0 8px 0; font-size: 0.95rem;">🏪 Store Metrics</h4>
                <p style="color: #4a4a4a; margin: 4px 0; font-size: 0.85rem;">
                    <b>Active Rate</b>: {active_rate:.0f}% ({active_stores} active / {inactive_stores} inactive)
                </p>
                <p style="color: #4a4a4a; margin: 4px 0; font-size: 0.85rem;">
                    <b>Avg Revenue/Store</b>: ₹{avg_rev/1e5:.1f}L per month
                </p>
            </div>
            <div style="background: #f5f3ef; border: 1px solid #e5e0d8; border-radius: 12px; padding: 16px;">
                <h4 style="color: #1e3a5f; margin: 0 0 8px 0; font-size: 0.95rem;">💰 Financial Range</h4>
                <p style="color: #4a4a4a; margin: 4px 0; font-size: 0.85rem;">
                    <b>Revenue Range</b>: ₹{rev_min/1e5:.1f}L – ₹{rev_max/1e5:.1f}L per store-month
                </p>
                <p style="color: #4a4a4a; margin: 4px 0; font-size: 0.85rem;">
                    <b>GM% Range</b>: {gm_min:.1f}% – {gm_max:.1f}%
                </p>
            </div>
            <div style="background: #f5f3ef; border: 1px solid #e5e0d8; border-radius: 12px; padding: 16px;">
                <h4 style="color: #1e3a5f; margin: 0 0 8px 0; font-size: 0.95rem;">📈 Profitability</h4>
                <p style="color: #4a4a4a; margin: 4px 0; font-size: 0.85rem;">
                    <b>EBITDA% Range</b>: {ebitda_min:.1f}% – {ebitda_max:.1f}%
                </p>
                <p style="color: #4a4a4a; margin: 4px 0; font-size: 0.85rem;">
                    <b>Profitable Stores</b>: {prof_count} / {total_count} store-months
                </p>
            </div>
        </div>
        """, unsafe_allow_html=True)
...
def render_variance_distribution(fdf: pd.DataFrame):
    """Render variance distribution charts and revenue bucket distribution."""
    ...
    with col1:
        fig1 = go.Figure(go.Bar(
            ...
            marker_color=[bucket_colors.get(b, NAVY) for b in var_counts.index],
            opacity=0.85,
    ...
    with col2:
        ...
        for i, bucket in enumerate(var_month.columns):
            color = bucket_colors.get(bucket, CHART_COLORS[i % len(CHART_COLORS)])
    ...
    rev_colors = {
        "(a) Below INR 15 lacs": BRICK, 
        "(b) INR 15 to 25 lacs": AMBER,
        "(c) INR 25 to 35 lacs": TEAL, 
        "(d) INR 35 to 45 lacs": NAVY,
        "(e) Above INR 45 lacs": FOREST
    }

    fig3 = go.Figure(go.Bar(
        x=rev_counts.index, y=rev_counts.values,
        marker_color=[rev_colors.get(b, NAVY) for b in rev_counts.index],
        opacity=0.85,
        hovertemplate="%{x}<br>Stores: %{y}<extra></extra>"
    ))
    fig3.update_layout(
...
def render_variance_analysis(fdf: pd.DataFrame):
    """Render detailed variance analysis with scatter plot and city-wise breakdown."""
    ...
    fig_city.add_trace(go.Bar(
        ...
        orientation="h", marker_color=NAVY, opacity=0.85,
...
def render_variance_data(fdf: pd.DataFrame):
    """Render the variance data table with formatted values and a download button."""
    st.markdown('<div class="section-header">📋 Variance Data</div>', unsafe_allow_html=True)

    display_cols = [
        "MONTH_STR", "STORE", "CITY", "NET REVENUE", "VARIANCE", "VARIANCE%", 
...
def main():
    """Launch the Kitchen P&L Intelligence Dashboard."""
    # Load data
    try:
        ...
        st.markdown("""
        <div style="background: #f5f3ef; border: 1px solid #e5e0d8; border-radius: 12px; padding: 20px; margin-top: 20px;">
            <h4 style="color: #1e3a5f; margin: 0 0 12px 0;">📋 Expected Data Structure</h4>
            <p style="color: #4a4a4a; font-size: 0.85rem; margin: 4px 0;">
